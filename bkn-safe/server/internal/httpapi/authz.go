...
func registerAuthz(r *gin.Engine, e *authz.Enforcer, db *gorm.DB, auditStore *audit.Store, dir *directory.Service) {
	...
	//
	// visibility_operations filters: visibility_match selects all (the default)
	// or any. An empty list returns every requested resource.
	// include_operations selects whether the response omits operations entirely,
	// or returns every operation registered for that resource type that the
	// accessor effectively holds.
	//
...
func registerRoleBindings(g *gin.RouterGroup, e *authz.Enforcer, db *gorm.DB) {
	...
	g.POST("/role-bindings", RequirePermission(e, "admin-role", "members"), func(c *gin.Context) {
		...
		err = e.Transaction(c.Request.Context(), func(tx *authz.PolicyTransaction) error {
