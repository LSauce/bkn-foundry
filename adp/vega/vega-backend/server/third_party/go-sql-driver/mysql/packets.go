...
func (mc *mysqlConn) initCapabilities(serverCapabilities capabilityFlag, serverExtCapabilities extendedCapabilityFlag, cfg *Config) {
	...
	if cfg.OceanBaseOracle {
		clientCapabilities &^= clientMySQL
		clientCapabilities |= clientIgnoreSpace | clientPSMultiResults | clientSessionTrack
		clientCapabilities |= clientSupportOracle
	}
	if cfg.compress {
	...
	if cfg.OceanBaseOracle {
		// OceanBase does not advertise this client-only marker in its greeting,
		// but Oracle-mode tenants require clients to send it in the response.
		mc.capabilities |= clientSupportOracle
	}
...
func (rows *textRows) readRow(dest []driver.Value) error {
	...
	for i := range dest {
		...
		case fieldTypeOceanBaseOracleTimestamp:
			dest[i], err = parseOceanBaseOracleTimestamp(buf, mc.cfg.Loc)
...
func parseOceanBaseOracleTimestamp(buf []byte, loc *time.Location) (time.Time, error) {
	if len(buf) < 12 {
		return time.Time{}, fmt.Errorf("invalid OceanBase Oracle TIMESTAMP length %d", len(buf))
	}
	year := int(buf[0])*100 + int(buf[1])
	nanos := int(binary.LittleEndian.Uint32(buf[7:11]))
	return time.Date(year, time.Month(buf[2]), int(buf[3]), int(buf[4]), int(buf[5]), int(buf[6]), nanos, loc), nil
}
